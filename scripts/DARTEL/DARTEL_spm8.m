...
clear all; home; 
addpath /jukebox/tamir/pkg/SPM8
addpath /jukebox/tamir/pkg/NIfTI
...
vox_size=2.0;               % voxel size at which to re-sample functionals (isotropic)

smooth_FWHM=0;              % smoothing kernel (isotropic)



% folder/directory information

owd = '/jukebox/tamir/prsonpipe/';                     % study directory

output=[owd filesep 'scripts' filesep 'DARTEL_output'];     % dir in which to save dartel output
...
subdirID='subjects';                   % subjects directory containing subject folders

niiDir=['data' filesep 'nifti'];    % folder where subject data are (from preprocessing pipeline)

subID='1*';                         % pattern for finding subject folders (use wildcards)

subTAG = 'all';                     % do which subjects? ('all' to do all, position vector, e.g. 1:4, to do a subset)

runID='*topup';                     % pattern for finding functional run files (use wildcards)
...
mprageID = '*anat.nii';   % pattern for finding matched-bandwidth image (use wildcards)
...
TPMimg= '/jukebox/tamir/pkg/SPM8/toolbox/Seg/TPM.nii';
...
for s = 1:length(dosubs)
    ...
    if funcFormat==1
        runs=[cbusub runID '.img'];
    else
        runs=[cbusub runID '.nii'];
    end
    ...
    for i = 1:numruns                                                     
        ...
        if funcFormat==1
            ...
            tmpString=sprintf('^%s.%s\\.nii$',cbusub,runID);   % name of epi functional runs
