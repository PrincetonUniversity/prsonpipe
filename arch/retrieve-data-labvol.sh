...
if [ ! -e ${RUNORDER_FILE} ]; then
	echo "Creating run order file for ${SUBJ}"
	cp ${PROJECT_DIR}/arch/subpars/runorder/run-order.txt ${RUNORDER_FILE}.txt
else
	echo "Run order file for ${SUBJ} already exists"
fi

echo "Run order is:"
cat ${RUNORDER_FILE} | grep "^[^#;]"
echo "###################################################"
echo "Please check the run order for this subject. Edit ${PROJECT_DIR}/arch/subject_pars/run_order/${SUBJ} if incorrect."
